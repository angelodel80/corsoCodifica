...
<xsl:stylesheet version="2.0"
                ...
                exclude-result-prefixes="xs xdt tv str"
    ...
    <span class="title">
      <xsl:choose>
        <xsl:when test="string(Series)">
          <xsl:call-template name="link">
            <xsl:with-param name="href" as="xs:anyURI" select="xs:anyURI(concat('#', Series))" />
            <xsl:with-param name="content" as="xs:string" select="key('IDs', Series)/Title" />
          </xsl:call-template>
          <xsl:if test="string(Title)">
            <xsl:text> - </xsl:text>
