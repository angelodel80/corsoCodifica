...
\begin{frame}
	...
	\begin{block}{XXXXXXXXXXXXXXXX}
		...
		% so it can be m­odified to match the needs of the document author and the data being recorded
	\end{block}
...
\begin{frame}
	...
	\begin{block}{XML Element: hierarchical relationship}
		\begin{itemize}
			\item Nested %element is a child element of the element in which it is nested: its parent element
			\item Elements %that are side by side in a document ­hierarchy are sibling elements
		\end{itemize}
	\end{block}
...
\begin{frame}
	...
	\begin{block}{XML Element: hierarchical relationship}
		\begin{itemize}
			\item All %elements in the body are children of a single element called the root element or document element.
			\item An %XML document must include a root element to be considered well formed
			\item hierarchy %represented in a tree diagram
			\item XML % declaration and comments are not included in the tree structure of the document body
			\item Syntax %error in creating an XML document is improperly nesting
			\item XML does % not allow the opening and ­closing tags of parent and child elements to overlap
		\end{itemize}
	\end{block}
...
\begin{frame}
	...
	\begin{block}{XML Element: Mixed Content}
		%An element  can contain both content and child elements.\\
		%Mixed content elements are ideal for descriptive, text-based chunks of information. 
	\end{block}
...
\begin{frame}
	...
	\begin{block}{XML PCDATA}
		%symbols such as \&, <, or >, which are all used in ­creating markup tags or entity references, are extracted
	\end{block}
...
\begin{frame}
	...
	\begin{block}{White Space: esempio}
		\begin{itemize}
			\item White% space is ignored when it is the only character data between element tags
			\item White% space is ignored within a document prolog and epilog, and within any element tags
			\item White% space within an attribute value is treated as part of the attribute value
			\item no% white space ­stripping occurs for element content, which means that the content of the XML element
		\end{itemize}
	\end{block}
