...
def sin(d):
  ...
  <xsl:template match="store">
    ...
    <xsl:variable name="currentAngle">
      <xsl:choose>
        <xsl:when test="function-available('jython-extension:cos')">
          <xsl:value-of 
    ...
    <svg:path fill="{$color}" stroke="black" stroke-width="2px" 
      ...
      <xsl:attribute name="transform">
        <xsl:choose>
          <xsl:when test="$explode">
            ...
            <xsl:text>,</xsl:text>
            <xsl:value-of 
              select="(jython-extension:sin(number($currentAngle div 2)) * -20) + 160"/>
            <xsl:text>) </xsl:text>
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:attribute name="d">
        ...
        <xsl:text>0 </xsl:text>
        <xsl:value-of select="jython-extension:cos(number($currentAngle)) * 80"/>
        <xsl:text> </xsl:text>
        <xsl:value-of select="jython-extension:sin(number($currentAngle)) * -80"/>
