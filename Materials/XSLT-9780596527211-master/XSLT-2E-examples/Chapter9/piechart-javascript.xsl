...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="store">
    ...
    <xsl:variable name="currentAngle">
      <xsl:choose>
        <xsl:when test="function-available('javascript-extension:cos')">
          <xsl:value-of 
    ...
    <svg:path fill="{$color}" stroke="black" stroke-width="2px" 
      ...
      <xsl:attribute name="transform">
        <xsl:choose>
          <xsl:when test="$explode">
            ...
            <xsl:value-of 
              select="(javascript-extension:cos(number($currentAngle div 2)) * 20) + 100"/>
            <xsl:text>,</xsl:text>
            <xsl:value-of 
              select="(javascript-extension:sin(number($currentAngle div 2)) * -20) + 160"/>
            <xsl:text>) </xsl:text>
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:attribute name="d">
        ...
        <xsl:text>0 </xsl:text>
        <xsl:value-of select="javascript-extension:cos(number($currentAngle)) * 80"/>
        <xsl:text> </xsl:text>
        <xsl:value-of select="javascript-extension:sin(number($currentAngle)) * -80"/>
