...
<xsl:variable name="Channels" as="element(Channel)+">
  <xsl:perform-sort select="trace(/TVGuide/Channel, 'sorting channels')">
    <xsl:sort select="trace(avg(Program/@rating), '  average rating')" order="descending" />
    <xsl:sort select="trace(xs:integer(Program[1]/@rating), '  first program rating')" order="descending" />
  </xsl:perform-sort>
</xsl:variable>
